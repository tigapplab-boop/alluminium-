...
import { useFournisseurs, useCreateFournisseur } from '@/hooks/useFournisseurs'
...
function FournisseurCard({
  ...
  return (
    <Card
      ...
      role="link"
      tabIndex={0}
      aria-label={`Voir le fournisseur ${fournisseur.nom}`}
      onKeyDown={(e) => {
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            ...
            {fournisseur.telephone && (
              <p className="text-xs text-alu-sub mt-0.5 flex items-center gap-1">
                <Phone className="h-3 w-3" />
                {fournisseur.telephone}
              </p>
            )}
            {fournisseur.wilaya && (
              <p className="text-xs text-alu-muted mt-1 flex items-center gap-1">
          ...
          <div className="flex flex-col items-end gap-1.5 shrink-0">
            <div className="flex items-center gap-1.5">
              {fournisseur._count?.achats !== undefined && (
                <Badge variant="EN_ATTENTE">
                  {fournisseur._count.achats} achat
                  {fournisseur._count.achats > 1 ? 's' : ''}
                </Badge>
              )}
              {fournisseur._count?.produits !== undefined &&
                fournisseur._count.produits > 0 && (
                  <Badge variant="ENVOYE">
                    {fournisseur._count.produits} produit
                    {fournisseur._count.produits > 1 ? 's' : ''}
                  </Badge>
                )}
            </div>
          </div>
...
function FournisseurCardSkeleton() {
  return (
    <Card>
      <CardContent className="p-4 sm:p-5">
        <div className="flex items-start justify-between gap-3">
...
export default function FournisseursPage() {
  ...
  const { data, isLoading, isError, error, refetch } = useFournisseurs(
    search || undefined
  )
  const createFournisseur = useCreateFournisseur()

  // Handle both ApiResponse and PaginatedResponse shapes
  const fournisseurs = (data?.data as Fournisseur[] | undefined) || []
  ...
  const onCreateSubmit = async (formData: FournisseurFormData) => {
    try {
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
          <div className="mb-4 rounded-full bg-alu-border/50 p-4">
            <Truck className="h-8 w-8 text-alu-muted" />
          </div>
          ...
          <p className="mt-1.5 max-w-sm text-sm text-alu-sub">
            {search
              ...
              : "Vous n'avez pas encore enregistré de fournisseur. Commencez par créer votre premier fournisseur."}
          </p>
          {!search && (
            <Button
              size="sm"
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)}>
        ...
        <DialogFooter>
          <Button
            ...
            disabled={createFournisseur.isPending}
          >
            Annuler
          </Button>
          <Button
            type="submit"
            form="create-fournisseur-form"
            loading={createFournisseur.isPending}
          >
