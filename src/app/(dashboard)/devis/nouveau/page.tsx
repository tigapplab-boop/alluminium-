  ...
  const onSubmit = async (data: DevisFormData) => {
    try {
      const result = await createDevis.mutateAsync({
        ...
        dateValidite: data.dateValidite || undefined,
        tva: Number(data.tva),
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input
                ...
                max={today}
                {...register('dateDevis')}
                error={errors.dateDevis?.message}
              />
              <Input
                label="Date de validité (optionnel)"
                type="date"
        ...
        <div className="flex justify-end">
          <Button type="submit" size="lg" loading={createDevis.isPending}>
